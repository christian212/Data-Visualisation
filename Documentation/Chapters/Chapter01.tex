...
example my unwise combination of {KOMA} and 
\texttt{titlesec} etc.).
\graffito{An outlook into the future of \texttt{classicthesis}.}

\begin{figure}
	\centering
	\tikzsetnextfilename{filter-algorithm}
	\includegraphics[width=\textwidth]{Tikz/filter-algorithm.tikz}
	\caption{Test caption.}
	\label{fig:filter-algorithm}
\end{figure}
...
\begin{figure}
\centering
\begin{tikzpicture}[every node/.style={transform shape}, auto, rounded corners=1pt, node distance=5cm,>=latex']
    
\node [block, align=center] (init) {Initialisation of parameters \\[2mm]  $\bm{x}_{0}, \bm{P}_{0}, \bm{H}, \bm{Q}, \bm{R}_{0},$};
\node [block, align=center, below of=init, node distance=3.8cm] (predict) {\emph{Time update} \\[2mm] Compute fundamental matrix: \\ $\bm{\Phi}^{[1]}_{k-1} \approx \bm{I}_{n} + \bm{F}_{k-1} T_s$ \\ Compute \emph{a priori} estimate: \\ $\hat{\bm{x}}^{-}_k = \hat{\bm{x}}_{k-1} + \bm{f}(\hat{\bm{x}}_{k-1})T_s$ \\ Compute \emph{a priori} error covariance: \\ $\bm{P}^-_{k} = \bm{\Phi}^{[1]}_{k-1} \bm{P}_{k-1} \bm{\Phi}^{[1]T}_{k-1} + \bm{Q}_{k-1}$};
\node [block, align=center, below of=predict, node distance=6.65cm] (measurement) {\emph{Correct sensor readings} \\[2mm] Compute acceleration due to motion: \\ $a_x = -l_1 [\omega^2_1 \cos(\theta_1) + \alpha_1 \sin(\theta_1)] - l_2 [(\omega_1 + \omega_2)^2$ \\ $\mathrel{\phantom{}}\cdot \cos(\theta_1 + \theta_2) + (\alpha_1 + \alpha_2) \sin(\theta_1 + \theta_2)]$ \\ $a_z = -l_1 [\alpha_1 \cos(\theta_1) - \omega^2_1 \sin(\theta_1)] - l_2 [(\alpha_1 + \alpha_2)$ \\ $\mathrel{\phantom{=ii}} \cdot \cos(\theta_1 + \theta_2) + (\omega_1 + \omega_2)^2 \sin(\theta_1 + \theta_2)]$ \\ Compute gravity estimate: \\ $\bm{g} \approx 
    \left[ \begin{smallmatrix}
    a_{X_2m} \\
    0 \\
    a_{Z_2m} 
    \end{smallmatrix} \right] -
    \bm{T}_y(\theta_1 + \theta_2 + 90^{\circ}) \left[ \begin{smallmatrix}
  	a_x \\
  	0 \\
	a_z
  \end{smallmatrix} \right] \|\bm{g}\|^{-1}$ \\ Compute corrected angle estimate: \\ $\theta_1 + \theta_2 = \mbox{atan}2(g_z, g_x)-180^{\circ}$ \\ Set measurement covariances: \\ $\sigma^2_3 = \begin{cases}
...
\begin{itemize}
 ...
 %  examples you want to use for your text.
    ...
    dedication, etc.
    \item\texttt{FIgures/} is where you put all the graphics you use in
