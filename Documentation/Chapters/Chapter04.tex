...
Consider, as an example involving filter theory, the continuous-time dynamical system depicted in Figure \ref{fig:state_estimation}. The desired state vector of the system, $\bm{x}(t)$, is usually hidden and can only be observed by indirect measurements $\bm{z}(t)$ that are a function of $\bm{x}(t)$ and subject to noise. Equally, the equation describing the evolution of the state $\bm{x}(t)$ is usually subject to system errors. These could be caused by, for instance, effects not accounted for in the model. The dynamical system may be an aircraft in flight, in which case the elements of the state vector are constituted by its position and velocity. The measuring system may be a tracking radar producing the observation vector $\bm{z}(t)$ over an interval $[0, T]$. The requirement of the filter is to deliver a reliable \emph{estimate} $\hat{\bm{x}}(t)$ of the actual state, by taking the measurement as well as a prior information into account.
...
\begin{tikzpicture}[auto, thick, rounded corners=1pt, node distance=3cm,>=latex']
    ...
    \draw [->, align=center] (dynamical) -- node[name=x] {State \\ $\bm{x}(t)$} (measuring);
    \draw [->, align=center] (measuring) -- node[name=y] {Observation \\ $\bm{z}(t)$} (estimator);
    \draw [->, align=center] (estimator) -- node[name=y] {Estimate \\ of state \\ $\hat{\bm{x}}(t)$} (output);
\end{tikzpicture}
}
...
\node [block, rounded corners=1pt, align=left, right of=input, node distance=4.4cm] (filter) {Linear \\ discrete-time \\ filter \\ $h_0, h_1, h_2, \dots$};
...
\draw [draw,->, align=left] (input) -- node [pos=0.29]{Input \\ $z(0), z(1), z(2), \dots$} (filter);
\draw [draw,->, align=left] (filter) -- node {Output \\ $\hat{x}(k)$} node[pos=0.9, label={below:$-$}] {} (sum);
\draw [draw,->, align=left] (output) -- node [label={above:Desired \\ response \\ $d(k)$}] {} node[pos=0.9] {$+$} (sum);
\draw [draw,->, align=left] (sum) -- node {Estimation \\ error \\ $e(k)$} (error);
...
\end{tikzpicture}
\caption{Conditional probability density of the position based on data $z_1$ and $z_2$, from \cite{Maybeck79}.} \label{fig:combination}
\end{figure}
...
\begin{tikzpicture}[pile/.style={->, >=stealth'}]
  ...
  \draw[-] (8,0) -- (8,1.94) node[pos=-0.15] {$\hat{x}^-(t_3)$};
  \draw[pile, style={->, >=stealth', shorten <=0pt, shorten
    >=0pt}] (8,0.8) -- (10.07,0.8) node[below, pos=0.5] {$\sigma^-_x(t_3)$};
  ...
  \draw[pile, style={dashed, ->, >=stealth', shorten <=0pt, shorten
    >=0pt}] (2,4) -- (8,4) node[above, pos=0.75] {$u$};
...
The conditional density of the position at time $t_2$, given $z_1$ and $z_2$, was previously derived. Figure \ref{fig:propagation} shows graphically how the density travels along the $x$-axis as time progresses. It starts at the best estimate and moves according to the above mentioned model of dynamics. Due to the constant addition of uncertainty over time it spreads out. Thus, as the variance increases, you become less sure of your position. The Gaussian density $f_{x(t_3)|z(t_1), z(t_2)}(x|z_1, z_2)$ can be expressed mathematically by its mean and variance given by
...
\begin{tikzpicture}[auto, thick, node distance=1.5cm,>=latex']
    ...
    \node [block, align=center, right of=phi1, node distance=2.2cm] (delay1) {$z^{-1_{ }}\bm{I}_n$};
    ...
    \node [block, below of=x_k, node distance=1.93cm] (delay) {$z^{-1_{ }}\bm{I}_n$};
    ...
    \draw [->] (sum2) -- node[] {$\bm{z}_k$} node[label={below:$+$}, pos=0.79] {} (sum3);
    ...
    \draw [->] (phi1) -- node[pos=0.27, name=h-phi] {} node[] {$\hat{\bm{x}}^-_k$} (H1);
    \draw [->] (delay1) -- node[] {$\hat{\bm{x}}_{k-1}$} node[] {} (phi1);
...
\node [block, align=center] (init) {Initialisation of parameters \\[3mm] $\bm{P}_{0}, \bm{x}_{0}, \bm{H}_{0}, \bm{\Phi}_{0}, \bm{Q}_{0}, \bm{R}_{0},$};
\node [block, align=center, below of=init, node distance=3.2cm] (predict) {\emph{Time update} \\[3mm] Compute \emph{a priori} estimate: \\ $\hat{\bm{x}}^-_k = \bm{\Phi}_{k-1}\hat{\bm{x}}_{k-1}+\bm{B}_{k-1}\bm{u}_{k-1}$ \\ Compute \emph{a priori} error covariance: \\ $\bm{P}^-_{k} = \bm{\Phi}_{k-1} \bm{P}_{k-1} \bm{\Phi}^T_{k-1} + \bm{Q}_{k-1}$};
\node [block, align=center, below of=predict, node distance=4.3cm] (update) {\emph{Measurement update} \\[3mm] Compute Kalman gain: \\ $\bm{K}_{k} = \bm{P}^-_k \bm{H}^T_k[\bm{H}_k \bm{P}^-_k \bm{H}^T_k + \bm{R}_k]^{-1}$ \\ Compute \emph{a posteriori} estimate: \\ $\hat{\bm{x}}_k = \hat{\bm{x}}^-_k + \bm{K}_{k}[\bm{z}_k-\bm{H}_{k}\hat{\bm{x}}^-_k]$ \\ Update error covariance: \\ $\bm{P}_{k} = [\bm{I}_n - \bm{K}_{k}\bm{H}_{k}]\bm{P}^-_{k}$};
...
\node [block, align=center] (init) {Initialisation of parameters \\[3mm] $\bm{x}_{0}, \bm{P}_{0}, \bm{Q}_{0}, \bm{R}_{0},$};
\node [block, align=center, below of=init, node distance=4cm] (predict) {\emph{Time update} \\[3mm] Compute \emph{a priori} estimate: \\ $\hat{\bm{x}}^{-}_k = \bm{\phi}_{k-1}(\bm{x}_{k-1}, \bm{u}_{k-1})$ \\ Compute Jacobian matrix: \\ $\bm{\Phi}^{[1]}_{k-1} =  \left. \frac{\partial \bm{\phi}_{k-1}(\bm{x}, \bm{u})}{\partial \bm{x}} \right|_{\bm{x}=\hat{\bm{x}}_{k-1}, \bm{u} = \bm{u}_{k-1}}$ \\ Compute \emph{a priori} error covariance: \\ $\bm{P}^-_{k} = \bm{\Phi}^{[1]}_{k-1} \bm{P}_{k-1} \bm{\Phi}^{[1]T}_{k-1} + \bm{Q}_{k-1}$};
\node [block, align=center, below of=predict, node distance=5.8cm] (update) {\emph{Measurement update} \\[3mm] Compute Jacobian matrix: \\ $\bm{H}^{[1]}_{k} = \left. \frac{\partial \bm{h}_{k}(\bm{x})}{\partial \bm{x}} \right|_{\bm{x}=\hat{\bm{x}}_{k}}$ \\ Compute Kalman gain: \\ $\bm{K}_{k} = \bm{P}^-_k \bm{H}^{[1]T}_k[\bm{H}^{[1]}_k \bm{P}^-_k \bm{H}^{[1]T}_k + \bm{R}_k]^{-1}$ \\ Compute \emph{a posteriori} estimate: \\ $\hat{\bm{x}}_k = \hat{\bm{x}}^-_k + \bm{K}_{k}[\bm{z}_k-\bm{h}_{k}(\hat{\bm{x}}^-_k)]$ \\ Update error covariance: \\ $\bm{P}_{k} = [\bm{I}_n - \bm{K}_{k}\bm{H}^{[1]}_{k}]\bm{P}^-_{k}$};
